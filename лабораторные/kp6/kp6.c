...
typedef struct Computer {
    ...
    int processors;
    char proc_type[str_len];
    int memory_size;
    char video_controller_type[str_len];
    int vid_mem_size;
    char HMDD_type[str_len];
    int HMDD_number;
    int HMDD_size;
    int controllers_number;
    int devices_number;
    char OS[str_len];
} comp;
...
void print() {
    ...
    if (thisfile == NULL) {
        ...
        while (fread(&pc, sizeof(comp), 1, thisfile) && feof(thisfile) == 0) {
            ...
            pc.surname, pc.processors, pc.proc_type, pc.memory_size, pc.video_controller_type, pc.vid_mem_size, pc.HMDD_type, pc.HMDD_number, pc.HMDD_size, pc.controllers_number, pc.devices_number, pc.OS);
        }
        fclose(thisfile);
        printf("------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
    }
...
void func() {
    ...
    if (thisfile == NULL)
        ...
        while (fread(&pc, sizeof(comp), 1, thisfile) && feof(thisfile) == 0) {
            if
            ((((strcmp(pc.proc_type, type1) == 0) || (strcmp(pc.proc_type, type2) == 0) || (strcmp(pc.proc_type, type3) == 0) || (strcmp(pc.proc_type, type4) == 0)) &&
            ((strcmp(pc.OS, os1) == 0) || (strcmp(pc.OS, os2) == 0) || (strcmp(pc.OS, os3) == 0) || (strcmp(pc.OS, os4) == 0) ||
            (strcmp(pc.OS, os5) == 0) || (strcmp(pc.OS, os6) == 0) || (strcmp(pc.OS, os7) == 0) || (strcmp(pc.OS, os8) == 0) || (strcmp(pc.OS, os9) == 0) || (strcmp(pc.OS, os10) == 0) || (strcmp(pc.OS, os11) == 0))) &&
            ((pc.processors > 1) || (pc.memory_size >= 16) || (pc.HMDD_number > 1) || (pc.HMDD_size >= 1024) || (pc.devices_number > 15))) {
                fprintf(stdout, "|%-15s|%-14d|%-16s|%-6d|%-19s|%-12d|%-12s|%-12d|%-10d|%-12d|%-7d|%-14s|\n",
                pc.surname, pc.processors, pc.proc_type, pc.memory_size, pc.video_controller_type, pc.vid_mem_size, pc.HMDD_type, pc.HMDD_number, pc.HMDD_size, pc.controllers_number, pc.devices_number, pc.OS);
            }
        }
